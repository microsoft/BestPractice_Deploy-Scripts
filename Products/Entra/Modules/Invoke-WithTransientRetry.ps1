...
<#
.SYNOPSIS
    Single retry boundary for transient Entra API and cmdlet operations.

.DESCRIPTION
    Uses bounded exponential backoff with small random jitter and records
    Started, Retried, Succeeded, and Failed outcomes through the Entra
...
function Invoke-WithTransientRetry {
    ...
    param(
        ...
        # Extra status codes to treat as transient for this call only. Used for
        # read-after-write verification, where a freshly created object can
        # briefly return 404 (Not Found) until directory replication catches up.
        [int[]] $RetryStatusCodes = @(),
        [int] $BackoffCapSeconds = 30,
        [int] $MaxJitterSeconds = 3
    )

    $transientStatusCodes = @(408, 429, 500, 502, 503, 504) + @($RetryStatusCodes) | Select-Object -Unique
    ...
    $logEnabled = [bool](Get-Command 'Add-EntraRunLogEntry' -ErrorAction SilentlyContinue)
    $stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
    if ($logEnabled) {
        Add-EntraRunLogEntry -Module 'Invoke-WithTransientRetry' `
    ...
    for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
        try {
            ...
            $errorRecord = $_
            $status = Get-EntraHttpStatusCode -ErrorRecord $errorRecord
            ...
            $transient = Test-EntraTransientFailure -ErrorRecord $errorRecord `
...
function Test-EntraTransientFailure {
    <#
    ...
    while ($exception) {
        if ($exception.GetType().Name -in $script:EntraTransientExceptionTypeNames) { return $true }
        $exception = $exception.InnerException
    }
    return ([string] $ErrorRecord.Exception.Message) -match $script:EntraTransientMessagePattern
}

function Get-EntraHttpStatusCode {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
