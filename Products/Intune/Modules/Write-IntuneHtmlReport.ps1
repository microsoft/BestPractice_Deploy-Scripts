...
# Get-IntuneModuleVerdict and the identity redaction helpers live in
# IntuneRunLog.ps1 so the JSON sidecar and this report use the same verdict and
# privacy rules. Callers dot-source that file before invoking this function.
function Write-IntuneHtmlReport {
    [CmdletBinding()]
    param(
    ...
    try {
        ...
        $moduleRows = foreach ($group in ($Entries | Group-Object Module | Sort-Object Name)) {
            $items = @($group.Group)
            $verdict = Get-IntuneModuleVerdict -Entries $items
...
<header><h1>Microsoft Intune Best Practice Deployment Report</h1><p>Read-only and state-changing evidence for one deployment run.</p></header>
...
<dt>Tenant ID</dt><dd>$(Encode (Protect-IntuneTenantId -Value $TenantId))</dd>
<dt>Tenant administrator</dt><dd>$(Encode (Protect-IntuneIdentity -Value $TenantAdminUpn))</dd>
...
"@
        ...
        throw "Unable to write Intune HTML report '$Path': $($_.Exception.Message)"
    }
}
